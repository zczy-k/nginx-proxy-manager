...
configure_nginx() {
    ...
    # 管理面板配置 (后端固定 3000 端口)
    # 前端 base.ts 给所有 API 调用加 /api/ 前缀，nginx 用 trailing slash 去掉前缀
    # 例: GET /api/tokens → proxy_pass → GET /tokens (后端路由无 /api/ 前缀)
...
server {
    ...
    # API: 前端 /api/* → 后端 /* (trailing slash 去除 /api/ 前缀)
    location /api/ {
